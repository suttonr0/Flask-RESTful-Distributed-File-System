...
def run():
    ...
    ip = 'localhost'
    port = 5000
    clientLibrary.listFiles(ip, port, localCache)
    fileObt = clientLibrary.getFile(ip, port, 'file1.txt', localCache)
    input("get test 1")
    clientLibrary.printFile(fileObt)
    input("print test 2")
    clientLibrary.listFiles(ip, port, localCache)
    input("list test 3")
    clientLibrary.editFile('file1.txt', "CacheTest1", localCache)
    input("edit test 4")
    clientLibrary.listFiles(ip, port, localCache)
    input("list test 5")
